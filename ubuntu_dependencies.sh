...
if command -v conda >/dev/null 2>&1; then
        ...
        CONDA_BIN="${CONDA_EXE}"
elif [[ -x "${TARGET_HOME}/miniconda/bin/conda" ]]; then
        CONDA_BIN="${TARGET_HOME}/miniconda/bin/conda"
        ...
        echo ""
        echo "If conda is already installed, start a new shell or run one of:"
...
if ! conda_cmd run -n "${ENV_NAME}" python --version >/dev/null 2>&1; then
...
conda_cmd install -y -n "${ENV_NAME}" -c conda-forge \
        "python=${PYTHON_VERSION}" \
        pip \
        numpy \
        scipy \
