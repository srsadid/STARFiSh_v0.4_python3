...
namespace
{
bool valuesMatchForCachedSolve(const std::vector<double*>& currentPointers, const std::vector<double>& cachedValues)
{
    if (currentPointers.size() != cachedValues.size())
    {
        return false;
    }

    for (std::size_t ii = 0; ii < currentPointers.size(); ii++)
    {
        const double currentValue = *currentPointers.at(ii);
        const double cachedValue = cachedValues.at(ii);
        const bool bothAreNaN = std::isnan(currentValue) && std::isnan(cachedValue);
        if (!(currentValue == cachedValue || bothAreNaN))
        {
            return false;
        }
    }
...
void copyPointerValuesForCachedSolve(const std::vector<double*>& currentPointers, std::vector<double>& cachedValues)
{
    cachedValues.clear();
    cachedValues.reserve(currentPointers.size());
    for (std::size_t ii = 0; ii < currentPointers.size(); ii++)
    {
        cachedValues.push_back(*currentPointers.at(ii));
    }
}
}
...
void NetlistCircuit::initialisePetscArrayNames()
{
    ...
    m_singleColumnRHS = PETSC_NULL;
    m_singleColumnSolution = PETSC_NULL;
...
void NetlistCircuit::terminatePetscArrays()
{
    ...
    if (m_singleColumnRHS)
    {
        errFlag = VecDestroy(&m_singleColumnRHS); CHKERRABORT(PETSC_COMM_SELF,errFlag);
    }
    if (m_singleColumnSolution)
    {
        errFlag = VecDestroy(&m_singleColumnSolution); CHKERRABORT(PETSC_COMM_SELF,errFlag);
    }
}
...
void NetlistCircuit::initialiseHistoryWritersIfNeeded()
{
	if (mp_pressureHistoryWriter == NULL)
	{
		mp_pressureHistoryWriter = new BasicFileWriter();
		mp_pressureHistoryWriter->setFileName(m_PressureHistoryFileName);
	}

	if (mp_flowHistoryWriter == NULL)
	{
		mp_flowHistoryWriter = new BasicFileWriter();
		mp_flowHistoryWriter->setFileName(m_FlowHistoryFileName);
	}

	if (mp_volumeHistoryWriter == NULL)
	{
		mp_volumeHistoryWriter = new BasicFileWriter();
		mp_volumeHistoryWriter->setFileName(m_VolumeHistoryFileName);
	}
}
...
void NetlistCircuit::writePressuresFlowsAndVolumes(int& nextTimestepWrite_start)
{
	...
	// nextTimestepWrite_start will be updated and returned to the caller by reference.
	if (nextTimestepWrite_start >= nextTimestepWrite_end)
	{
		return;
	}

	try {
		initialiseHistoryWritersIfNeeded();
		{
			// Write the netlistPressures_zeroDDomainReplacement.dat
			for (int stepToWrite=nextTimestepWrite_start; stepToWrite<nextTimestepWrite_end; stepToWrite++)
			{
				mp_pressureHistoryWriter->writeStepIndex(stepToWrite);
				for (auto node=mp_circuitData->mapOfPressureNodes.begin(); node!=mp_circuitData->mapOfPressureNodes.end(); node++)
				{
					mp_pressureHistoryWriter->writeToFile(node->second->getFromPressureHistoryByTimestepIndex(stepToWrite));
				}
				mp_pressureHistoryWriter->writeEndLine();
			}
			mp_pressureHistoryWriter->flush();
		}

		{
			...
			for (int stepToWrite=nextTimestepWrite_start; stepToWrite<nextTimestepWrite_end; stepToWrite++)
			{
				mp_flowHistoryWriter->writeStepIndex(stepToWrite);
				for (auto component=mp_circuitData->components.begin(); component!=mp_circuitData->components.end(); component++)
				{
					mp_flowHistoryWriter->writeToFile((*component)->getFromFlowHistoryByTimestepIndex(stepToWrite));
				}
				mp_flowHistoryWriter->writeEndLine();
			}
			mp_flowHistoryWriter->flush();
		}

		{
			...
			for (int stepToWrite=nextTimestepWrite_start; stepToWrite<nextTimestepWrite_end; stepToWrite++)
			{
				mp_volumeHistoryWriter->writeStepIndex(stepToWrite);
				for (auto component=mp_circuitData->mapOfComponents.begin(); component!=mp_circuitData->mapOfComponents.end(); component++)
				{
					VolumeTrackingComponent* volumeTrackingComponent = dynamic_cast<VolumeTrackingComponent*> (component->second.get());
					// If this component is actually a volume chamber, so it actually has a volume history we can write to the file:
					if (volumeTrackingComponent != NULL)
					{
						mp_volumeHistoryWriter->writeToFile(volumeTrackingComponent->getVolumeHistoryAtTimestep(stepToWrite));
					}
				}
				mp_volumeHistoryWriter->writeEndLine();
			}
			mp_volumeHistoryWriter->flush();
		}
	...
	// Set the starting place for the write on the next iteration.
...
{
    invalidateCachedLinearSystemSolution();
...
{
  PetscErrorCode errFlag;
  // Create the dense system matrix used by the direct solve.
  ...
  // Scratch vectors reused for the extra unit-vector solve used by implicit coefficients.
  errFlag = VecDuplicate(m_RHS,&m_singleColumnRHS); CHKERRABORT(PETSC_COMM_SELF,errFlag);
  errFlag = VecZeroEntries(m_singleColumnRHS); CHKERRABORT(PETSC_COMM_SELF,errFlag);
  errFlag = VecAssemblyBegin(m_singleColumnRHS); CHKERRABORT(PETSC_COMM_SELF,errFlag);
  errFlag = VecAssemblyEnd(m_singleColumnRHS); CHKERRABORT(PETSC_COMM_SELF,errFlag);

  errFlag = VecDuplicate(m_solutionVector,&m_singleColumnSolution); CHKERRABORT(PETSC_COMM_SELF,errFlag);
  errFlag = VecZeroEntries(m_singleColumnSolution); CHKERRABORT(PETSC_COMM_SELF,errFlag);
  errFlag = VecAssemblyBegin(m_singleColumnSolution); CHKERRABORT(PETSC_COMM_SELF,errFlag);
  errFlag = VecAssemblyEnd(m_singleColumnSolution); CHKERRABORT(PETSC_COMM_SELF,errFlag);
}
...
{
    if (cachedLinearSystemSolutionIsReusable(alfi_delt,true))
    {
        return;
    }
    ...
    solveLinearSystem();
    updateCachedLinearSystemSolutionMetadata(alfi_delt,true);
}

void NetlistCircuit::buildAndSolveLinearSystem(const double alfi_delt)
{
  if (cachedLinearSystemSolutionIsReusable(alfi_delt,false))
  {
    return;
  }
  ...
  solveLinearSystem();
  updateCachedLinearSystemSolutionMetadata(alfi_delt,false);
}

void NetlistCircuit::solveLinearSystem()
{
  PetscErrorCode errFlag;
  solveFactorizedLinearSystem(m_RHS,m_solutionVector);
  errFlag = MatSetUnfactored(m_systemMatrix); CHKERRABORT(PETSC_COMM_SELF,errFlag);
}

void NetlistCircuit::solveFactorizedLinearSystem(const Vec rhs, Vec solution)
{
  PetscErrorCode errFlag;
  errFlag = MatSolve(m_systemMatrix,rhs,solution); CHKERRABORT(PETSC_COMM_SELF,errFlag);
}

void NetlistCircuit::releaseFactorizedLinearSystem()
{
  PetscErrorCode errFlag;
  errFlag = MatSetUnfactored(m_systemMatrix); CHKERRABORT(PETSC_COMM_SELF,errFlag);
}

void NetlistCircuit::invalidateCachedLinearSystemSolution()
{
    m_hasCachedLinearSystemSolution = false;
    m_cachedBoundaryPressures.clear();
    m_cachedBoundaryFlows.clear();
}

bool NetlistCircuit::cachedLinearSystemSolutionIsReusable(const double alfi_delt, const bool useHistoryHistoryPressure) const
{
    if (!m_hasCachedLinearSystemSolution)
    {
        return false;
    }

    if (m_cachedAlfiDelt != alfi_delt)
    {
        return false;
    }

    if (m_cachedUsedHistoryHistoryPressure != useHistoryHistoryPressure)
    {
        return false;
    }

    if (m_cachedOneshotIgnoreIncorrectFortranFlow != m_oneshotIgnoreIncorrectFortranFlow)
    {
        return false;
    }

    if (!valuesMatchForCachedSolve(pressure_n_ptrs,m_cachedBoundaryPressures))
    {
        return false;
    }

    if (!valuesMatchForCachedSolve(flow_n_ptrs,m_cachedBoundaryFlows))
    {
        return false;
    }
...
void NetlistCircuit::updateCachedLinearSystemSolutionMetadata(const double alfi_delt, const bool useHistoryHistoryPressure)
{
    m_hasCachedLinearSystemSolution = true;
    m_cachedAlfiDelt = alfi_delt;
    m_cachedUsedHistoryHistoryPressure = useHistoryHistoryPressure;
    m_cachedOneshotIgnoreIncorrectFortranFlow = m_oneshotIgnoreIncorrectFortranFlow;
    copyPointerValuesForCachedSolve(pressure_n_ptrs,m_cachedBoundaryPressures);
    copyPointerValuesForCachedSolve(flow_n_ptrs,m_cachedBoundaryFlows);
}
...
std::pair<double,double> NetlistCircuit::computeImplicitCoefficients(const int timestepNumber, const double timen_1, const double alfi_delt)
{
    ...
    const bool canReuseCachedStandardSolve = cachedLinearSystemSolutionIsReusable(alfi_delt,false);
    generateLinearSystemFromPrescribedCircuit(alfi_delt);
    if (!canReuseCachedStandardSolve)
    {
        assembleRHS(false);
        solveFactorizedLinearSystem(m_RHS,m_solutionVector);
        updateCachedLinearSystemSolutionMetadata(alfi_delt,false);
    ...
    // linear solve
    assert(m_locationOf3DInterfaceComputedPressureInSolutionVector.size() == 1);
    assert(m_columnOf3DInterfacePrescribedFlowInLinearSystem.size() == 1);
    const int interfacePressureRow = m_locationOf3DInterfaceComputedPressureInSolutionVector.at(0);
    const int interfaceFlowColumn = m_columnOf3DInterfacePrescribedFlowInLinearSystem.at(0);
    int rowToGet[] = {interfacePressureRow};
    int numberOfValuesToGet=1;
    PetscScalar valueFromUnitResponseSolve;
    
    std::pair<double,double> implicitCoefficientsToReturn;

    PetscErrorCode errFlag;
    PetscScalar valueFromRHS;
    errFlag = VecGetValues(m_RHS,numberOfValuesToGet,&interfaceFlowColumn,&valueFromRHS);CHKERRABORT(PETSC_COMM_SELF,errFlag);
    ...
    // We only need one column of A^{-1}: solve A y = e_j and read the interface-pressure row of y.
    errFlag = VecZeroEntries(m_singleColumnRHS); CHKERRABORT(PETSC_COMM_SELF,errFlag);
    errFlag = VecSetValue(m_singleColumnRHS,interfaceFlowColumn,1.0,INSERT_VALUES); CHKERRABORT(PETSC_COMM_SELF,errFlag);
    errFlag = VecAssemblyBegin(m_singleColumnRHS); CHKERRABORT(PETSC_COMM_SELF,errFlag);
    errFlag = VecAssemblyEnd(m_singleColumnRHS); CHKERRABORT(PETSC_COMM_SELF,errFlag);
    solveFactorizedLinearSystem(m_singleColumnRHS,m_singleColumnSolution);
    errFlag = VecGetValues(m_singleColumnSolution,numberOfValuesToGet,rowToGet,&valueFromUnitResponseSolve); CHKERRABORT(PETSC_COMM_SELF,errFlag);
    releaseFactorizedLinearSystem();

    implicitCoefficientsToReturn.first = mp_circuitData->getSignForPrescribed3DInterfaceFlow() * valueFromUnitResponseSolve;
    
    implicitCoefficientsToReturn.second = valueFromSolutionVector - valueFromUnitResponseSolve * valueFromRHS;//\todo make dynamic
    
    // std::cout << "unit response solve: "<< std::endl;
    // VecView(m_singleColumnSolution,PETSC_VIEWER_STDOUT_WORLD);
