...
public:
	...
	m_alfi(alfi),
		...
		mp_pressureHistoryWriter(NULL),
		mp_flowHistoryWriter(NULL),
		mp_volumeHistoryWriter(NULL),
		m_hasCachedLinearSystemSolution(false),
		m_cachedAlfiDelt(0.0),
		m_cachedUsedHistoryHistoryPressure(false),
		m_cachedOneshotIgnoreIncorrectFortranFlow(false)
	{
	...
	{
		delete mp_pressureHistoryWriter;
		delete mp_flowHistoryWriter;
		delete mp_volumeHistoryWriter;
...
protected:
	...
	int m_NumberOfAtomicSubcircuits;
		...
		void initialiseHistoryWritersIfNeeded();

		Mat m_systemMatrix;
		Vec m_RHS;
		Vec m_solutionVector;
		Vec m_singleColumnRHS;
		Vec m_singleColumnSolution;
	...
	std::vector<int> m_locationOf3DInterfaceComputedFlowInSolutionVector;
		...
		BasicFileWriter* mp_pressureHistoryWriter;
		BasicFileWriter* mp_flowHistoryWriter;
		BasicFileWriter* mp_volumeHistoryWriter;
		bool m_hasCachedLinearSystemSolution;
		double m_cachedAlfiDelt;
		bool m_cachedUsedHistoryHistoryPressure;
		bool m_cachedOneshotIgnoreIncorrectFortranFlow;
		std::vector<double> m_cachedBoundaryPressures;
		std::vector<double> m_cachedBoundaryFlows;
...
private:
	...
	void setupCustomPythonControlSystems();
		...
		void solveFactorizedLinearSystem(const Vec rhs, Vec solution);
		void releaseFactorizedLinearSystem();
		void invalidateCachedLinearSystemSolution();
		bool cachedLinearSystemSolutionIsReusable(const double alfi_delt, const bool useHistoryHistoryPressure) const;
		void updateCachedLinearSystemSolutionMetadata(const double alfi_delt, const bool useHistoryHistoryPressure);
