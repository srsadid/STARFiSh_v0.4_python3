...
class RandomInputManager(TestBaseClass):
    ...
    def initialize(self, vascularNetwork):
        ...
        for name,randomInput in self.randomInputs.items():
            # set name            
    ...
    def linkRandomInputUpdateFunctions(self, vascularNetwork):
        ...
        randomInputMap = {}
        for randomInputName,randomInput in self.randomInputs.items():
        ...
        for dist,updateMethods in randomInputMap.items():
            # find random input with dist:
            for randomInputName in self.randomInputs.keys():
                if dist == randomInputName:
                    ...
                    generalRandomInput.variableName = list(updateMethods.keys())
    ...
    def saveRealisationLog(self, evaluationLogFile, networkName, dataNumber, caseName): 
        ...
        logData = np.array([randomInput.updateLog for randomInput in list(self.randomInputs.values())]).transpose()
        ...
        randomInputName = [randomInput.name for randomInput in list(self.randomInputs.values())]
    ...
    def generateInfo(self):
        ...
        randomInputManagerInfo.append( "-------------------------------------------------------------------- \n")
        randomInputInfos = list(itertools.chain.from_iterable([randomInput.generateInfo() for randomInput in list(self.randomInputs.values())]))
